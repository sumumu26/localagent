...
別プロジェクト「ハコブネ（箱舟）」に乗せて運ぶエージェントをイメージしています。ハコブネの上に複数の arko が乗り込み、それぞれが役割を持ちながら協力して作業する——そういった **複数エージェントの共同作業** を見据えた設計思想を込めた名前です。arko 単体としては、外部のLLMに依存せずどこへでも持ち運べるポータブルなローカルLLMエージェントです。
