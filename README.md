...
arko（アークオー）は、英語の "ark"（方舟）から派生した造語で、**A portable local LLM agent** を意味します。

方舟がエージェントを乗せて運び、あなたのそばで共同作業できるようにする——外部のLLMに依存せず、どこへでも持ち運べるエージェントを作りたい、という思いから名付けました。
...
| 透明性 | **ループ全体を一読できる** | フレームワーク内部が複雑 | SDK抽象化で挙動が見えにくい |
...
- **LLMエージェントの仕組みを理解したい** — フレームワークなしのシンプルなReActループで、読んで動かしてすぐ学べる
...
- **KVキャッシュ量子化対応** — `--kv-cache-type-k/v` で f16/q8_0/q4_0 を指定可能。TurboQuant fork ビルド時は tbq4_0（3.94x圧縮）・tbq3_0（5.22x圧縮）も使用でき、16GB VRAM + Qwen3.5-9B で最大〜435K contextを実現
...
### TurboQuant KV キャッシュ量子化を使う場合（オプション）

`--kv-cache-type-k tbq4_0` / `--kv-cache-type-k tbq3_0` を使うには、llama.cpp 本家への PR #21089 がまだマージ待ちのため、コミュニティ fork からビルドが必要です。

```bash
# TurboQuant fork のビルド（CUDA の場合）
...
# Apple Silicon の場合
pip install -e . --no-build-isolation \
    -C cmake.args="-DGGML_METAL=ON"
```

> **注意**: fork ビルドを使わない場合（標準の llama-cpp-python）でも、`tbq4_0`/`tbq3_0` を指定した場合は警告を出して f16 にフォールバックするため、起動は正常に行われます。
...
| `--kv-cache-type-k` | `f16` | KV キャッシュの K 側量子化型（f16 / q8_0 / q4_0 / tbq4_0 / tbq3_0）|
| `--kv-cache-type-v` | `f16` | KV キャッシュの V 側量子化型（同上）|
...
# TurboQuant 4-bit KV キャッシュ（fork ビルド必要）
# 16GB VRAM + Qwen3.5-9B で ~335K context
python main.py \
  --model ~/models/Qwen3.5-9B-Q4_K_M.gguf \
  --n-ctx 131072 \
  --kv-cache-type-k tbq4_0 \
  --kv-cache-type-v tbq4_0

# TurboQuant 3-bit KV キャッシュ（fork ビルド必要、最大圧縮）
# 16GB VRAM + Qwen3.5-9B で ~435K context
python main.py \
  --model ~/models/Qwen3.5-9B-Q4_K_M.gguf \
  --n-ctx 131072 \
  --kv-cache-type-k tbq3_0 \
  --kv-cache-type-v tbq3_0

# 非対称: K は精度重視 tbq4_0、V は q8_0（標準ビルドで q8_0 は利用可能）
python main.py \
  --model ~/models/Qwen3.5-9B-Q4_K_M.gguf \
  --n-ctx 65536 \
  --kv-cache-type-k tbq4_0 \
...
├── requirements.txt
└── agent/
    ├── llm.py               # llama-cpp-pythonラッパー（コンテキスト圧縮・KVキャッシュ量子化含む）
